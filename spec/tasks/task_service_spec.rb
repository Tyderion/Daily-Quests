require 'spec_helper'

describe TaskService do

  before :each do
    @titles = ["Should not be found","Search term anywhere", "Search term is where we look","Where is the term?" ]
    @term = "whe"
    @titles.each do |title|
      FactoryGirl.create(:task, title: title )
    end
  end


  describe "search_by_title" do
    it "returns an array of tasks containing the search title" do
      subject.search_by_title(@term).map{|ta| ta.title }.should == @titles[1,3]
    end
  end

  describe "sort_by" do
    before :each do
      @result = subject.search_by_title(@term)
    end
    it "first are titles that begin with the search term" do
      subject.sort_by(@result, @term)[0].title.should == @titles[3]
    end
    it "second are titles that have a word that begins with the search term" do
      subject.sort_by(@result, @term)[1].title.should == @titles[2]
    end
    it "third are titles that have the term anywhere" do
      subject.sort_by(@result, @term)[2].title.should == @titles[1]
    end
    it "is the same size as the array passed in" do
      subject.sort_by(@result, @term).length.should == @result.length
    end
  end


end
